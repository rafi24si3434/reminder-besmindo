...
<div class="space-y-6">
    ...
    <!-- Two Column: Left Interactive Simulator, Right CSV Import -->
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">

        <!-- Left: Interactive Simulator Buttons (7 Cols) -->
                ...
                <div class="space-y-2.5 max-h-[480px] overflow-y-auto pr-1 scrollbar-thin">
                    <?php foreach ($attendances as $att): ?>
                        <div class="p-3 bg-slate-800/50 border border-slate-700/60 rounded-xl flex items-center justify-between gap-3">
                            <div class="truncate">
                                <div class="font-bold text-white text-xs truncate"><?= esc($att['crew_name']) ?></div>
                                <div class="text-[10px] text-slate-400 truncate"><?= esc($att['position']) ?> &bull; Status: <strong class="text-sky-300"><?= $att['status'] ?></strong></div>
                            </div>
        ...
        <!-- Right: Teams Attendance CSV Import (5 Cols) -->
        <div class="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <form action="<?= base_url('attendance/importCsv') ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
                <?= csrf_field() ?>
