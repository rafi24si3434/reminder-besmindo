...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center space-x-2">
                ...
                <h1 class="text-2xl font-black text-white tracking-tight"><?= esc($meeting['title']) ?></h1>
            </div>
            <p class="text-xs text-slate-400 mt-1">Detail jadwal, penanggung jawab, dan daftar peserta crew rig</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <a href="<?= esc($meeting['teams_link']) ?>" target="_blank" class="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs flex items-center space-x-2 shadow-lg shadow-indigo-600/30 transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div class="flex flex-wrap items-center gap-2">
                <span class="px-3 py-1 rounded-md text-xs font-bold bg-sky-500/20 text-sky-300 border border-sky-500/30">
                    <i class="fa-solid fa-oil-well mr-1.5"></i> <?= esc($meeting['rig_name']) ?> (<?= esc($meeting['rig_code']) ?>)
                </span>
                ...
                <?php if ($meeting['is_recurring']): ?>
                    <span class="px-3 py-1 rounded-md text-xs font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                        <i class="fa-solid fa-repeat mr-1.5"></i> Rutin Setiap <?= esc($meeting['recurring_day']) ?>
                    </span>
                <?php endif; ?>
            ...
            <div class="space-y-1">
                <h2 class="text-xl font-bold text-white"><?= esc($meeting['title']) ?></h2>
                <p class="text-xs text-slate-300 bg-slate-800/60 p-3 rounded-xl border border-slate-700/60">
                    ...
                    <?= nl2br(esc($meeting['topic'] ?: 'Tidak ada deskripsi topik.')) ?>
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2 text-xs">
                ...
                <div class="bg-slate-800/40 p-3 rounded-xl border border-slate-700/40 col-span-2 sm:col-span-1">
                    ...
                    <strong class="text-white text-xs truncate block"><?= esc($meeting['rig_location']) ?></strong>
                </div>
            </div>
            ...
            <div class="p-3 bg-slate-800/30 border border-slate-700/40 rounded-xl flex items-center justify-between text-xs">
                <div class="truncate mr-2">
                    ...
                    <span class="text-sky-400 font-mono truncate block"><?= esc($meeting['teams_link']) ?></span>
                </div>
                <a href="<?= esc($meeting['teams_link']) ?>" target="_blank" class="px-3 py-1.5 rounded-lg bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-300 font-semibold text-xs flex-shrink-0">
                    Buka Teams &rarr;
                </a>
            </div>
        </div>

        <!-- Right 1 Col: PJ Rig & Status Meeting -->
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <div class="flex items-center space-x-3 p-3.5 bg-slate-800/50 border border-slate-700/50 rounded-xl">
                ...
                <div>
                    <h4 class="text-sm font-bold text-white"><?= esc($meeting['pj_name'] ?? 'Belum ditentukan') ?></h4>
                    <p class="text-[11px] text-slate-400"><?= esc($meeting['pj_position'] ?? 'PJ Rig') ?></p>
                </div>
            </div>
            ...
            <?php if (!empty($meeting['pj_phone'])): ?>
                <div class="text-xs text-slate-400 flex items-center justify-between">
                    ...
                    <a href="https://wa.me/<?= esc($meeting['pj_phone']) ?>" target="_blank" class="text-emerald-400 font-mono hover:underline flex items-center space-x-1">
                        <i class="fa-brands fa-whatsapp"></i>
                        <span>+<?= esc($meeting['pj_phone']) ?></span>
                    </a>
                </div>
            <?php endif; ?>
            ...
            <div class="pt-4 border-t border-slate-800 space-y-2">
                ...
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <a href="<?= base_url('meeting/updateStatus/' . $meeting['id'] . '/in_progress') ?>" class="px-3 py-1.5 rounded-lg text-center bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 font-semibold border border-emerald-500/30 transition">
                        Mulai Meeting
                    </a>
                    <a href="<?= base_url('meeting/updateStatus/' . $meeting['id'] . '/completed') ?>" class="px-3 py-1.5 rounded-lg text-center bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold border border-slate-700 transition">
                        Tandai Selesai
                    </a>
                </div>
            </div>
    ...
    <!-- Section: Attendance & Participant List -->
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                        ...
                        <?php foreach ($attendances as $att): ?>
                            <tr class="hover:bg-slate-800/40 transition">
                                ...
                                <td class="px-4 py-3 text-right">
                                    ...
                                    ?>
                                    <button type="button" onclick="openWhatsApp('<?= $att['phone'] ?>', '<?= esc($msg, 'js') ?>')" 
