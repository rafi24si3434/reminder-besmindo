    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-lg">
        <form method="GET" action="<?= base_url('meeting') ?>" class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                ...
                <div>
                    <input type="date" name="date" value="<?= esc($selectedDate) ?>" onchange="this.form.submit()" class="bg-slate-800 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-2 focus:ring-2 focus:ring-sky-500 focus:outline-none">
                </div>
                    ...
                    <div class="space-y-2 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="px-2.5 py-0.5 rounded text-[11px] font-bold bg-sky-500/20 text-sky-300 border border-sky-500/30">
                                <i class="fa-solid fa-oil-well mr-1"></i> <?= esc($m['rig_name']) ?>
                            </span>
                            ...
                            <?php if ($m['is_recurring']): ?>
                                <span class="px-2.5 py-0.5 rounded text-[11px] font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                                    <i class="fa-solid fa-repeat mr-1"></i> Rutin Tiap <?= esc($m['recurring_day'] ?? 'Minggu') ?>
                                </span>
                            <?php else: ?>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-400 line-clamp-1 mt-0.5"><?= esc($m['topic']) ?></p>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center text-xs text-slate-400 gap-x-5 gap-y-1 pt-1">
                            ...
                            <span><i class="fa-solid fa-user-tie text-slate-500 mr-1.5"></i> PJ: <strong class="text-slate-200"><?= esc($m['pj_name'] ?? '-') ?></strong></span>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex flex-wrap items-center gap-2 pt-2 md:pt-0 border-t md:border-t-0 border-slate-800">
                        <a href="<?= esc($m['teams_link']) ?>" target="_blank" class="px-3 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 text-xs font-semibold flex items-center space-x-1.5 transition" title="Buka Microsoft Teams">
                        ...
                        <a href="<?= base_url('reminder/broadcast/' . $m['id']) ?>" class="px-3 py-2 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 text-xs font-semibold flex items-center space-x-1.5 transition" title="Broadcast Undangan WhatsApp">
                            <i class="fa-brands fa-whatsapp"></i>
                            <span class="hidden sm:inline">Kirim WA</span>
                        </a>

                        <div class="inline-flex items-center space-x-1 pl-1">
                            <button type="button" onclick="openQuickReschedule(<?= $m['id'] ?>, '<?= esc($m['title']) ?>', '<?= $m['meeting_date'] ?>', '<?= substr($m['start_time'], 0, 5) ?>', '<?= substr($m['end_time'], 0, 5) ?>')" 
                                class="p-2 text-slate-400 hover:text-amber-400 hover:bg-slate-800 rounded-lg transition" title="Ubah Jadwal Cepat (Reschedule)">
                                <i class="fa-solid fa-clock-rotate-left"></i>
                            </button>
                            <a href="<?= base_url('meeting/edit/' . $m['id']) ?>" class="p-2 text-slate-400 hover:text-sky-400 hover:bg-slate-800 rounded-lg transition" title="Edit Lengkap">
                                <i class="fa-regular fa-pen-to-square"></i>
                            </a>
                            <button type="button" onclick="confirmDelete(<?= $m['id'] ?>, '<?= esc($m['title']) ?>')" class="p-2 text-slate-400 hover:text-rose-400 hover:bg-slate-800 rounded-lg transition" title="Hapus Meeting">
...
<script>
    ...
    function submitQuickReschedule(e) {
        ...
        fetch('<?= base_url('meeting/quickReschedule') ?>', {
            method: 'POST',
            body: formData,
            headers: {
