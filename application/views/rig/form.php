        ...
        <form action="<?= base_url('rig/save') ?>" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea name="description" rows="3" placeholder="Informasi kapasitas horsepower, kedalaman pengeboran, dll." class="w-full px-4 py-2.5 bg-slate-800/80 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500 transition"><?= isset($rig['description']) ? htmlspecialchars($rig['description']) : '' ?></textarea>
                </div>
            </div>

            <div class="pt-2">
                <label class="inline-flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" <?= (!isset($rig['is_active']) || $rig['is_active'] == 1) ? 'checked' : '' ?> class="w-4 h-4 text-sky-600 bg-slate-800 border-slate-700 rounded focus:ring-sky-500">
