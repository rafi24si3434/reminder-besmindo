...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
            ...
            <div class="mt-4">
                <div class="text-3xl font-extrabold text-white"><?= count($todayMeetings) ?></div>
                <div class="flex items-center space-x-1.5 text-xs text-slate-400 mt-1">
                    <span><?= count($upcomingMeetings) ?> meeting terjadwal ke depan</span>
                </div>
            </div>
        </div>

        <!-- Card 2: Crew Hadir Tepat Waktu -->
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4">
                <div class="text-3xl font-extrabold text-emerald-400"><?= $attendanceStats['hadir'] ?> <span class="text-xs font-normal text-slate-400">Crew</span></div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
            ...
            <div class="mt-4">
                <div class="text-3xl font-extrabold text-amber-400">
                    <?= $attendanceStats['terlambat'] ?> <span class="text-sm font-normal text-slate-400">Late / <?= $attendanceStats['belum_hadir'] ?> Pending</span>
                </div>
                <div class="flex items-center space-x-1 text-xs text-amber-400/80 mt-1">
                    <i class="fa-solid fa-triangle-exclamation"></i>
                    <span>Memerlukan follow-up WA</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg relative overflow-hidden">
            ...
            <div class="mt-4">
                <div class="text-3xl font-extrabold text-indigo-400"><?= $attendanceStats['percentage'] ?>%</div>
                <div class="w-full bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                    <div class="bg-gradient-to-r from-sky-500 to-indigo-500 h-full rounded-full" style="width: <?= min(100, $attendanceStats['percentage']) ?>%"></div>
                </div>
            </div>
        </div>
    </div>

    <!-- Section: Meeting Hari Ini & Quick Live Monitor -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Left: Today's Meetings (2 cols) -->
        <div class="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <?php if (empty($todayMeetings)): ?>
                <div class="text-center py-10 border border-dashed border-slate-800 rounded-xl bg-slate-950/40">
                    ...
                    <p class="text-xs text-slate-500 mt-1">Anda dapat membuat jadwal meeting baru untuk crew rig.</p>
                    ...
                    <?php foreach ($todayMeetings as $m): ?>
                        <div class="bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 rounded-xl p-4 transition duration-150 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div class="space-y-1.5">
                                ...
                                <h3 class="text-sm font-bold text-white"><?= esc($m['title']) ?></h3>
                                <div class="flex flex-wrap items-center text-xs text-slate-400 gap-x-4 gap-y-1">
                                    ...
                                    <span><i class="fa-solid fa-user-tie mr-1 text-slate-500"></i> PJ: <?= esc($m['pj_name'] ?? '-') ?></span>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <a href="<?= esc($m['teams_link']) ?>" target="_blank" class="px-3 py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 border border-indigo-500/30 text-xs font-semibold flex items-center space-x-1.5 transition" title="Buka Meeting Teams">
                                ...
                                <a href="<?= base_url('reminder/broadcast/' . $m['id']) ?>" class="px-2.5 py-1.5 rounded-lg bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 border border-emerald-500/30 text-xs transition" title="Broadcast Undangan WhatsApp">
        ...
        <!-- Right: Status Unit Rig & Crew Summary (1 col) -->
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div class="flex items-center justify-between border-b border-slate-800 pb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-400">Total <?= $totalRigs ?> Rig Aktif &bull; <?= $totalCrews ?> Crew Terdaftar</p>
                </div>
                <a href="<?= base_url('rig') ?>" class="text-xs text-sky-400 hover:text-sky-300 font-medium">Kelola &rarr;</a>
            </div>

            <div class="space-y-3">
                <?php foreach ($rigSummaries as $r): ?>
                    <div class="p-3.5 bg-slate-800/50 border border-slate-700/50 rounded-xl space-y-2">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-xs font-mono px-2 py-0.5 rounded bg-slate-700 text-sky-300"><?= esc($r['code']) ?></span>
                        </div>
                        <div class="text-xs text-slate-400 flex items-center justify-between">
                            <span><i class="fa-solid fa-location-dot text-slate-500 mr-1"></i> <?= esc($r['location']) ?></span>
                        ...
                        <div class="text-[11px] text-slate-500 pt-1 border-t border-slate-700/40 flex items-center justify-between">
                            <span>PJ: <strong class="text-slate-300"><?= esc($r['pj_name']) ?></strong></span>
    ...
    <!-- Section: Attendance Chart & Quick WhatsApp Reminders Outbox -->
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <?php else: ?>
                <div class="space-y-2.5">
                    <?php foreach ($recentReminders as $log): ?>
                        <div class="p-3 bg-slate-800/40 border border-slate-700/50 rounded-xl flex items-center justify-between text-xs">
                            <div class="space-y-0.5">
                                <div class="flex items-center space-x-2">
                                    <span class="font-bold text-white"><?= esc($log['crew_name'] ?? 'Crew') ?></span>
                                    <span class="px-1.5 py-0.5 rounded text-[10px] font-mono bg-sky-500/20 text-sky-300"><?= esc($log['reminder_type']) ?></span>
                                </div>
                                <p class="text-slate-400 truncate max-w-xs text-[11px]"><?= esc($log['meeting_title'] ?? '') ?></p>
                            </div>
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    ...
                    '#64748b', // Slate (Belum Hadir)
                    '#f43f5e', // Rose (Tidak Hadir)
                    '#3b82f6'  // Blue (Izin)
                ],
                borderColor: '#0f172a',
                borderWidth: 3
