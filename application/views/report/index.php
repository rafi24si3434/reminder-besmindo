<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="<?= base_url('report/export_excel?' . http_build_query($_GET)) ?>" class="inline-flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs shadow-lg shadow-emerald-600/30 transition">
                <i class="fa-solid fa-file-excel"></i>
                <span>Download Excel (.CSV)</span>
            </a>
            <a href="<?= base_url('report/print_report?' . http_build_query($_GET)) ?>" target="_blank" class="inline-flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-semibold text-xs transition">
        ...
        <form method="GET" action="<?= base_url('report') ?>" class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                ...
                <div class="flex items-center space-x-2 text-xs text-slate-400">
                    <span>Dari:</span>
                    <input type="date" name="start_date" value="<?= htmlspecialchars($startDate ?: '') ?>" onchange="this.form.submit()" class="bg-slate-800 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-2 focus:ring-2 focus:ring-sky-500 focus:outline-none">
                </div>
                <div class="flex items-center space-x-2 text-xs text-slate-400">
                    <span>Sampai:</span>
                    <input type="date" name="end_date" value="<?= htmlspecialchars($endDate ?: '') ?>" onchange="this.form.submit()" class="bg-slate-800 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-2 focus:ring-2 focus:ring-sky-500 focus:outline-none">
                </div>
                            ...
                            <?php foreach ($mostAbsentCrews as $mac): ?>
                                <tr class="hover:bg-slate-800/40">
                                    <td class="px-3 py-2.5">
                                        <div class="font-bold text-white"><?= htmlspecialchars($mac['name']) ?></div>
                                        <div class="text-[10px] text-slate-400"><?= htmlspecialchars($mac['position']) ?></div>
                                    </td>
                ...
                <?php foreach ($rigRecaps as $rr): ?>
                    ...
                    <div class="p-3.5 bg-slate-800/50 border border-slate-700/60 rounded-xl space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-bold text-white"><?= htmlspecialchars($rr['name']) ?></span>
                    ...
                    <?php else: ?>
                        <?php foreach ($records as $rec): ?>
                            <tr class="hover:bg-slate-800/40">
                                ...
                                <td class="px-4 py-3">
                                    <div class="font-bold text-white truncate max-w-xs"><?= htmlspecialchars($rec['meeting_title']) ?></div>
                                    <div class="text-[10px] text-sky-400"><?= htmlspecialchars($rec['rig_name']) ?></div>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="font-bold text-white"><?= htmlspecialchars($rec['crew_name']) ?></div>
                                    <div class="text-[10px] text-slate-500 font-mono"><?= htmlspecialchars($rec['nik']) ?></div>
                                </td>
