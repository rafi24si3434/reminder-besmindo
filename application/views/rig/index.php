        ...
        <?php foreach ($rigs as $r): ?>
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col justify-between hover:border-slate-700 transition">
                <div class="space-y-4">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-white"><?= htmlspecialchars($r['name']) ?></h3>
                    ...
                    <?php if (!empty($r['description'])): ?>
                        <p class="text-xs text-slate-500 line-clamp-2"><?= htmlspecialchars($r['description']) ?></p>
                    <?php endif; ?>

                    <div class="pt-3 border-t border-slate-800/80 space-y-2 text-xs">
                        <div class="flex items-center justify-between text-slate-400">
                            <span>Penanggung Jawab (PJ):</span>
                            <strong class="text-slate-200"><?= htmlspecialchars($r['pj_name']) ?></strong>
                        </div>
                        <div class="flex items-center justify-between text-slate-400">
                ...
                <div class="pt-5 border-t border-slate-800/80 flex items-center justify-between mt-4">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <button type="button" onclick="confirmDelete(<?= $r['id'] ?>, '<?= htmlspecialchars($r['name']) ?>')" class="p-1.5 text-slate-400 hover:text-rose-400 hover:bg-slate-800 rounded-lg transition" title="Hapus Rig">
